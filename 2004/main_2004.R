...
library(haven)
library(dplyr)
setwd("/Users/nicolasleon/Documents/Uni/SoSe23/Data_final/Data_Inei/2004/input")
getwd()
...
add_id_to_df <- function(input_df) {
  input_df$AÑO <- as.numeric(input_df$AÑO)
  input_df$MES <- as.numeric(input_df$MES)
  input_df$CONGLOME <- as.numeric(input_df$CONGLOME)
  input_df$VIVIENDA <- as.numeric(input_df$VIVIENDA)
  input_df$HOGAR <- as.numeric(input_df$HOGAR)
  input_df$CODPERSO <- as.numeric(input_df$CODPERSO)
  input_df$UBIGEO <- as.numeric(input_df$UBIGEO)
  input_df$DOMINIO <- as.numeric(input_df$DOMINIO)
...
tipo_de_seguro <- function(input_df, col1, colname1) {
  input_df[[colname1]] <- ifelse(input_df[[col1]] == 1, "Si", ifelse(input_df[[col1]] == 0, "No", NA))
  return(input_df)
}
...
df <- read_sav("Enaho01-2004-200.sav")
...
df <- remove_rows_with_mostly_NAs(df)

output <- df


######### LENGUA MATERNA, EDUCACION ####
df <- read_sav("Enaho01a-2004-300.sav")
df$P300A
df <- df %>% 
...
df <- read_sav("Enaho01a-2004-400.sav")

df$P4193
df$ESSALUD_CONTRIBUCION

df <- tipo_de_seguro_y_contribucion(df, "P4191", "P420A", "ESSALUD")
df <- tipo_de_seguro_y_contribucion(df, "P4192", "P420B", "SEGURO_PRIVADO")
df <- tipo_de_seguro_y_contribucion(df, "P4193", "P420C", "ENTIDAD_PRESTADORA")
df <- tipo_de_seguro_y_contribucion(df, "P4194", "P420D", "SEGURO_FFAA_PNP")
df <- tipo_de_seguro_y_contribucion(df, "P4195", "P420E", "SIS")
...
df$CONTRATO <- ifelse((df$P511A < 7 | df$P507 == 8), "Si", "No")

df$SEGURO_PENSION <- ifelse(df$P558A == 2, "No", "Si") 

df$REGISTRO_IMPUESTOS_LUG_TRABAJO <- ifelse(df$P510A == 1 | df$P510B == 1, "Si", "No")
...
output$SEGURO_SALUD <- ifelse((output$ESSALUD == "Si" & (output$ESSALUD_CONTRIBUCION == "Centro de trabajo" | output$ESSALUD_CONTRIBUCION == "Mismo")) | 
                                ...
                                (output$SEGURO_UNIVERSITARIO == "Si") | 
                                (output$OTRO_SEGURO == "Si"), 
                              "Si", 
                              "No")
output$SEGURO_SOCIAL <- ifelse(output$SEGURO_PENSION == "Si" | output$SEGURO_SALUD == "Si", "Si", "No")
######### SAVE CSV ########
write.csv(output, "results_2004.csv", row.names = FALSE, fileEncoding = "UTF-8")
output <- read.csv("results_2004.csv", header = TRUE, sep = ",", dec = ".", check.names = FALSE, fileEncoding = "UTF-8", encoding = "UTF-8")
...
calculate_correlation(df, "SECTOR_FORMAL", "REGISTRO_IMPUESTOS_LUG_TRABAJO")

a_ver <- summary_table(df, ESTADO_CIVIL, CONTRATO)
